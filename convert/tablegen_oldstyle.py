...
print('//written '+ str(count) + 'hex words')
...
if (IMAGE.size[0] != 128) or (IMAGE.size[1] != 128):
    sys.stderr.write('Image size lid-lower.png must match screen size')
    exit(1)
...
try:
    ...
    if (IMAGE.size[0] != IRIS_SIZE) or (IMAGE.size[1] != IRIS_SIZE):
        sys.stderr.write('Image size pupilMap.png must match iris size ' + str( IRIS_SIZE) )
        exit(1)
    IMAGE = IMAGE.convert('L')
    PIXELS = IMAGE.load()
except IOError:
...
sys.stdout.write('\nconst uint16_t polar[%s*%s] PROGMEM= {' % (IRIS_SIZE, IRIS_SIZE))
